...
class MyPdf extends FPDF
{
    ...
    function header()
    {
        ...
        if (!$participant) {
            ...
            $this->image('font-end/assets/base/img/dossier/releve.jpg',10,11,198);
             ...
             $this->Cell(0,118,''.utf8_decode(" ".$participant->prenom." ".$participant->nom),0,0,'C');
        } 
    }
     ...
     function headerTable()
    {
        ...
        $this->Cell(10,0);
        $this->Cell(147,6,utf8_decode('Modules'),1,0,'L');

        $this->Cell(28,6,'Note sur 100',1,0,'L');
        $this->Ln();
    }
    ...
    function viewTable()
    {
        ...
        foreach (Module::liste($url[1]) as $module ){
            ...
            if($module->titre!='Introduction' ){
                $this->Cell(10,0);
            $this->Cell(147,6,utf8_decode($module->titre),1,0,'L');
            // couleur les note 
            if (!isset($quiz_test->nom)){
               $note='';
            }
            $this->Cell(28,6,$note,1,0,'L');
            $this->Ln();
            }
